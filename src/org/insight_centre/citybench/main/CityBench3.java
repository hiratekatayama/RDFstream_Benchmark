...
public class CityBench3 {
	...
	public static CQELSEngine test_3_59;
	public static CsparqlEngineImpl csparqlEngine;
	private static final Logger logger = LoggerFactory.getLogger(CityBench3.class);
	...
	public static void main(String[] args) {
		try {
			...
			// ここを確認
			CityBench3 cb = new CityBench3(prop, parameters);
			cb.startTest();
	 ...
	 * @throws Exception
	 */
	public CityBench3(Properties prop, HashMap<String, String> parameters) throws Exception {
		...
		try {
			...
			System.out.println(this.cqels_query);
		...
		try {
			...
			System.out.println("initializeDataset: " + dataset);
			//System.out.println("contextDictionary: " + tempContext.dictionary().all().next().getLeft());
	...
	private void initCQELS() throws Exception {
		...
		//System.out.println("contextDictionary: " + cqelsContext.dictionary().all().next().getLeft());
		//System.out.println("contextDictionary: " + cqelsContext.dictionary().all().next().getRight());
		//System.out.println("contextDictionary: " + cqelsContext.dictionary().all().next().hashCode());
		//test_3_59 = new CQELSEngine(cqelsContext);
		//NodeId dum = cqelsContext.dictionary().all().next().getLeft();
		//System.out.println("cqels NodeId: " + dum);
		//long dum_4_16 = cqelsContext.dictionary().all().next().getLeft().getId();
		//System.out.println("cqels NodeId: " + dum_4_16);
		//
		//Node dum_4_21 = test_3_59.decode(dum_4_16);

		//System.out.println("decode: " + dum_4_21.toString());
	...
	private void startCQELSStreams() throws Exception {
		for (String s : this.queryMap.values()) {
            //ここがCQELSエンジンの肝
            //System.out.println("CityBench queryMap: " + s);
	...
	private void startCQELSStreamsFromQuery(String query) throws Exception {
		...
		for (String sn : streamNames) {
			...
			//cqelsContextでは，CQELSengine.encodeで辞書型で識別子が管理されているのか調べる
			//CQELSEngineクラスのNodeってなに？
			if (!this.startedStreams.contains(uri)) {
				this.startedStreams.add(uri);
				//System.out.println("check startedStreamsName");
				//System.out.println(this.startedStreams);
				//ssこれ調べる
				CQELSSensorStream css;
				EventDeclaration ed = er.getEds().get(uri);
				//System.out.println("Citybench check uriNames: " + uri);
				//System.out.println("Citybench check pathNames: " + path);
				//System.out.println("Citybench check ed: " + ed);
				//System.out.println("Citybench check start: " + start);
				//System.out.println("Citybench check end: " + end);
				if (ed == null)
				...
				//System.out.println(startedStreamObjects.iterator().next());
				//for (Object i : startedStreamObjects)
				//	System.out.println("startedStreamObjects: " + i.getClass().getName());
				//Object dum_5_18 = startedStreamObjects.toArray().length;
				//System.out.println("startedStreamObjects1: " + dum_5_18);
				//Object dum_5_19 = startedStreamObjects.toArray();
				//System.out.println("startedStreamObjects2: " + dum_5_19);
			}
		}
