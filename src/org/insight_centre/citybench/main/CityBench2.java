...
public class CityBench2 {
	...
	private static final Logger logger = LoggerFactory.getLogger(CityBench2.class);
	public static ConcurrentHashMap<String, SensorObservation> obMap = new ConcurrentHashMap<String, SensorObservation>();

	//追加
	public static CqelsplusExecContext cqelsplusContext, tempplusContext;
	...
	public static void main(String[] args) {
		try {
			...
			// ここを確認
			CityBench2 cb = new CityBench2(prop, parameters);
			cb.startTest();
	 ...
	 * @throws Exception
	 */
	public CityBench2(Properties prop, HashMap<String, String> parameters) throws Exception {
	...
	private void registerCQELSplusQuery(String qid, String query) {
		if (!registeredQueries.keySet().contains(qid)) {
			...
			//ここですよ
			System.out.println(cqelsplusContext);
			QueryRouter cs = cqelsplusContext.engine().registerSelectQuery(query);
			//ContinuousSelect cs = cqelsplusContext.engine().registerSelectQuery(query);
			//cs.register(crl);
