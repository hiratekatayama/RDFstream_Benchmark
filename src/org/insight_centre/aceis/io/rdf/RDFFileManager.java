...
public class RDFFileManager {
	...
	public static ExecContext initializeCQELSContext(String serviceDesc, Reasoner r) {
		...
		//System.out.println("contextDictionary: " + context.dictionary().all().next().getRight());
        //System.out.println("contextDictionary: " + context.dictionary().all().next().getLeft());
		//System.out.println("contextDictionary: " + context.dictionary().all().next().hashCode());
		//System.out.println("engine: " + context.engine().toString());
		//while (dataset.listNames().hasNext())
		//	System.out.println(dataset.listNames().next());
		return context;
	}

	public static cqelsplus.engine.ExecContext initializeCQELSplusContext(String serviceDesc, Reasoner r) {
		cqelsplus.engine.CqelsplusExecContext context = null;
		if (r != null) {
			//コメントアウト
			context = new cqelsplus.engine.ReasonerContext(cqelsHome, true, r);
		} else {
			context = new CqelsplusExecContext(cqelsHome, true);
		}

		System.out.println("initalize CQELSplus Context: " + context);

		context.loadDefaultDataset(datasetDirectory + serviceDesc);
		context.loadDataset(cesPrefix, ontologyDirectory + "ces.n3");
		context.loadDataset("culturalevents", datasetDirectory + "AarhusCulturalEvents.n3");
		context.loadDataset("libraryevents", datasetDirectory + "AarhusLibraryEvents.n3");
		dataset = changeDataset((DatasetGraphTDB) context.getDataset());
		// context.loadDataset(ssnPrefix, ontologyDirectory + "ssn.owl");
		// context.loadDataset(ctPrefix, ontologyDirectory + "city.n3");
		// context.loadDataset(owlsPrefix, ontologyDirectory + "Service.owl");
		// context.loadDataset(owlsPrefix, ontologyDirectory + "Grounding.owl");
		...
		//datasetcqlesplus = context.changeDataset(context.getDataset());
		//System.out.println("initializeDataset: " + datasetcqlesplus);
		//System.out.println("contextDictionary: " + context.dictionary().all().next().getRight());
		//System.out.println("contextDictionary: " + context.dictionary().all().next().getLeft());
		//System.out.println("contextDictionary: " + context.dictionary().all().next().hashCode());
		//System.out.println("engine: " + context.engine().toString());
	...
	private static Dataset changeDataset(DatasetGraphTDB dataset) {
	...
	public static void writeRepoToFile(String fileName, EventRepository repo) throws IOException, NodeRemovalException {
		...
		for (EventDeclaration ed : repo.getEds().values()) {
			//System.out.println("RDFFileManager Creating ed " + i + ":" + ed.toString());
			serviceModel.add(createEDModel(serviceModel, ed));
			i++;
